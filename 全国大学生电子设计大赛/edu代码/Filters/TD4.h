...
static inline void TD4_Lite_reset( TD4_Lite* filter )
{
	filter->x1 = filter->x2 = filter->x3 = filter->x4 = 0;
}

static inline void TD4_Lite_init( TD4_Lite* filter )
{
	TD4_Lite_reset( filter );
}

static inline float TD4_Lite_track4( TD4_Lite* filter , const float expect , const float h , float P1 , float P2 , float P3 , float P4 )
{
	filter->tracking_mode = 4;
...
static inline void TD4double_Lite_reset( TD4double_Lite* filter )
{
	filter->x1 = filter->x2 = filter->x3 = filter->x4 = 0;
}

static inline void TD4double_Lite_init( TD4double_Lite* filter )
{
	TD4double_Lite_reset( filter );
}

static inline double TD4double_Lite_track4( TD4double_Lite* filter , const double expect , const double h , double P1 , double P2 , double P3 , double P4 )
{
	filter->tracking_mode = 4;
	
	double e1 = expect - filter->x1;
	double e1_1 = -filter->x2;
	double e1_2 = -filter->x3;
	double e1_3 = -filter->x4;
	double T2 = P1 * e1;
	double T2_1 = P1 * e1_1;
	double T2_2 = P1 * e1_2;
	double T2_3 = P1 * e1_3;
	
	double e2 = T2 - filter->x2;
	double e2_1 = T2_1-filter->x3;
	double e2_2 = T2_2-filter->x4;
	double T3 = P2 * e2;
	T3 += T2_1;
	double T3_1 = P2 * e2_1 + T2_2;
	double T3_2 = P2 * e2_2 + T2_3;
	
	double e3 = T3 - filter->x3;
	double e3_1 = T3_1-filter->x4;
	double T4 = P3 * e3;
	T4 += T3_1;
	double T4_1 = P3 * e3_1 + T3_2;
	
	double e4 = T4 - filter->x4;
