...
/*外置罗盘定义*/
	...
	/*  iic地址, 数据寄存器地址 , 高位在前, 手动采样 , 采样寄存器地址, 采样设置, ID 地址 , ID   , 配置寄存器地址 , 配置数目,  配置                                                     ,  灵敏度      , 采样率毫秒  , 轴向                 ,  */
		...
		{ 0x0d   ,      0         ,   false ,   false  ,            0  ,      0  ,      9  , 0x00 ,       0x9      , 3       , { (0<<6) | (0<<4) | (0b10<<2) | (1<<0) , (1<<6) , 1 }     , 8.33333333e-5, 10          , { -2 , -1 , -3 }   } , //QMC5883
...
//		i2c0WriteData( icm20602_Addr , 105 , buf , 1 );
	...
	/*IST8310 configurations*/
		//复位
		buf[0] = (0<<6) | (0<<4) | (0b10<<2) | (1<<0);
		i2c0WriteData( QMC5883_Addr , 0x9 , buf , 1 );
		delay( 0.1f );
		
		//开启16x内部平均
		buf[0] = (1<<6);
		i2c0WriteData( QMC5883_Addr , 0xa , buf , 1 );
		
		//Set/Reset内部平均
		buf[0] = 1;
		i2c0WriteData( QMC5883_Addr , 0xb , buf , 1 );
		
		//注册传感器
		IMUMagnetometerRegister( 2 , 8.33333333e-5f );
	/*IST8310 configurations*/
		...
		switch( i2c_current_read_sensor )
		{
			...
			case i2c_sensor_icm20602:
			{
				...
				else
				{
					...
					data_buffer[ i2c_read_counter - 1 ] = I2CMasterDataGet(I2C0_BASE);
					vector3_int data;
					data.x =  -(int32_t)( (int16_t)__REV16( *(uint16_t*)&data_buffer[0] ) );
					data.y =   (int32_t)( (int16_t)__REV16( *(uint16_t*)&data_buffer[2] ) );
					data.z =  -(int32_t)( (int16_t)__REV16( *(uint16_t*)&data_buffer[4] ) );
					IMUAccelerometerUpdate( 0 , data );
					data.x =  -(int32_t)( (int16_t)__REV16( *(uint16_t*)&data_buffer[8] ) );
					data.y =   (int32_t)( (int16_t)__REV16( *(uint16_t*)&data_buffer[10] ) );
					...
					else if( read_ist8310 )
					{
						...
						I2CMasterSlaveAddrSet(I2C0_BASE, QMC5883_Addr, false); // 设置寄存器地址（写模式）
						I2CMasterDataPut(I2C0_BASE, 0);
						I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_SEND_START); // 发送起始位 写入寄存器编号		
					}
					else
			...
			case i2c_sensor_ist8310:
			{
				...
				{				
					//已发送start 并写入了寄存器地址
					I2CMasterSlaveAddrSet(I2C0_BASE, QMC5883_Addr, true); // 再次设置寄存器地址（读模式）
					I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_RECEIVE_START); // 发送重复起始位 开始读数据				
				}
				else if( i2c_read_counter < 5 )
				{
