...
static inline void map_projection_init(double lat_0, double lon_0 , Map_Projection* m) //lat_0, lon_0 are expected to be in correct format: -> 47.1234567 and not 471234567
{
	...
	double d = sin(lat1) * sin_lat_2 + cos(lat1) * cos_lat_2 * cos(lon2 - lon1);
	if( d >= 1.0 )
		d = 0;
	else if( d <= -1.0 )
		d = M_PI;
	...
	double k_bar = 0;
	double c =  m->sin_phi_1 * sin_lat_2 + m->cos_phi_1 * cos_lat_2 * cos(lon2 - m->lambda_0);
	if( c >= 1.0 )
		c = 0;
	else if( c <= -1.0 )
		c = M_PI;
...
static void map_projection_project(double lat, double lon, double *x, double *y , const Map_Projection* m)
{
	...
	double c =  m->sin_phi_1 * sin_phi + m->cos_phi_1 * cos_phi * (1 - pow((lambda - m->lambda_0), 2) / 2); //double c =  acos( sin_phi_1 * sin_phi + cos_phi_1 * cos_phi * cos(lambda - lambda_0) );
	if( c >= 1.0 )
		c = 0;
	else if( c <= -1.0 )
		c = M_PI;
...
static void map_projection_reproject(float x, float y, double *lat, double *lon , const Map_Projection* m)
{
	...
	if (c != 0)
	{
		lat_sphere = cos_c * m->sin_phi_1 + (x_descaled * sin_c * m->cos_phi_1) / c;
		if( lat_sphere >= 1.0 )
			lat_sphere = M_PI / 2;
		else if( lat_sphere <= -1.0 )
			lat_sphere = -M_PI / 2;
		else
			lat_sphere = asin( lat_sphere );
	}
	else
	{
		lat_sphere = cos_c * m->sin_phi_1;
		if( lat_sphere >= 1.0 )
			lat_sphere = M_PI / 2;
		else if( lat_sphere <= -1.0 )
			lat_sphere = -M_PI / 2;
...
//	printf("lat_sphere = %.10f\n",lat_sphere);
	...
	if (m->phi_1  == M_PI * 0.5) {
		//using small angle approximation (formula in comment is without aproximation)
		//lon_sphere = (m->lambda_0 - y_descaled / x_descaled); 
		lon_sphere = (m->lambda_0 + atan2(-y_descaled, x_descaled));
