...
/*模式及期望*/
	...
	bool Attitude_Control_Enable()
	{
		...
		/*初始化*/
			...
			//初始化姿态ESO
			init_ESO_AngularRate( &ESO[0] , Cfg_get_MotorT() , Cfg_get_RPYCtrl_b(0) , 0.7f , 10 , 500 , 5000 );
			init_ESO_AngularRate( &ESO[1] , Cfg_get_MotorT() , Cfg_get_RPYCtrl_b(1) , 0.7f , 10 , 500 , 5000 );
			init_ESO_AngularRate( &ESO[2] , 1.0f/200 , Cfg_get_RPYCtrl_b(2) , 0.7f , 10 , 200 , 200 );
		/*初始化*/
...
//			out[i] *= 100;
//			if( out[i] < output_minimum_throttle )
//				out[i] = output_minimum_throttle;
			out[i] *= output_range;
			out[i] += output_minimum_throttle;			
		}
...
void init_ctrl_Attitude()
{
	...
	//电池电压ADC放大倍数初始设置
	Cfg_set_initial_BatVoltageADCMag( 10.5 );
