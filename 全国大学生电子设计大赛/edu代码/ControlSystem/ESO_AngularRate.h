...
static inline void init_ESO_AngularRate( ESO_AngularRate* eso , float T , float b , float beta1 , float beta2 , float ceta1 , float ceta2 )
{
	eso->beta1 = beta1;
	eso->beta2 = beta2;
...
static inline float ESO_AngularRate_run( ESO_AngularRate* eso , const float v , const float h )
{
	...
	float beta1_scale = 1 + eso->ceta1 * err_continues_time3;	
	float beta2_scale = 1 + eso->ceta2 * err_continues_time3;		
	if( beta1_scale > 15 )
		beta1_scale = 15;
	if( beta2_scale > 15 )
		beta2_scale = 15;
