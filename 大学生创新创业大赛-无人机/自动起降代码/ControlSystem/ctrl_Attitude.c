	...
	static inline void update_output_throttle( float throttle , float h )
	{
		...
		else
		{
			if( hover_throttle < output_minimum_throttle + 2 && \
					...
					in_symmetry_range_float( vector3_float_square( get_VelocityENU() ) , 10*10 ) \
				)
			{
...
/*模式及期望*/
	...
	bool Attitude_Control_Enable()
	{
		...
		/*初始化*/
			//初始化期望TD4滤波器
			TD4_init( &Target_tracker[0] , Cfg_get_PRYCtrl_TD4P( 0 ) , Cfg_get_PRYCtrl_TD4P( 0 ) , Cfg_get_PRYCtrl_TD4P( 0 ) , Cfg_get_PRYCtrl_TD4P( 0 ) );
			TD4_init( &Target_tracker[1] , Cfg_get_PRYCtrl_TD4P( 1 ) , Cfg_get_PRYCtrl_TD4P( 1 ) , Cfg_get_PRYCtrl_TD4P( 1 ) , Cfg_get_PRYCtrl_TD4P( 1 ) );
			TD4_init( &Target_tracker[2] , Cfg_get_PRYCtrl_TD4P( 2 ) , Cfg_get_PRYCtrl_TD4P( 2 ) , Cfg_get_PRYCtrl_TD4P( 2 ) , Cfg_get_PRYCtrl_TD4P( 2 ) );
			
			//初始化姿态ESO
			init_ESO_AngularRate( &ESO[0] , Cfg_get_PRYCtrl_T(0) , Cfg_get_PRYCtrl_b(0) , 0.7f , 15 );
			init_ESO_AngularRate( &ESO[1] , Cfg_get_PRYCtrl_T(1) , Cfg_get_PRYCtrl_b(1) , 0.7f , 15 );
			init_ESO_AngularRate( &ESO[2] , Cfg_get_PRYCtrl_T(2) , Cfg_get_PRYCtrl_b(2) , 0.7f , 15 );
		/*初始化*/
...
void ctrl_Attitude()
{
	...
	//获取控制参数
	float Ps = Cfg_get_PRYCtrl_P1(0);
	vector3_float P = { Cfg_get_PRYCtrl_P2(0) , Cfg_get_PRYCtrl_P2(1) , Cfg_get_PRYCtrl_P2(2) };
	...
	switch(Yaw_ControlMode)
	{
		case Attitude_ControlMode_Angle:
		{
			...
			{
				target_Yaw -= 2*PI_f;
				Target_tracker[2].x1 -= 2*PI_f;
			}
			else if( target_Yaw < -PI_f )
			{
				target_Yaw += 2*PI_f;
				Target_tracker[2].x1 += 2*PI_f;
			}
			...
			if( angle_error < -PI_f ) angle_error+=2*PI_f;
			else if( angle_error > PI_f ) angle_error-=2*PI_f;
			if( angle_error < -PI_f ) angle_error+=2*PI_f;
			else if( angle_error > PI_f ) angle_error-=2*PI_f;
	...
	//计算前馈量
		...
		vector3_float Ta1 = { P.x*( Tv1.x - angular_acceleration_ESO.x ) + Tv2.x ,
													P.y*( Tv1.y - angular_acceleration_ESO.y ) + Tv2.y ,
													P.z*( Tv1.z - angular_acceleration_ESO.z ) + Tv2.z };
	//计算前馈量
	...
	//计算目标角加速度
		...
		target_angular_acceleration.x *= P.x;
		target_angular_acceleration.y *= P.y;
		target_angular_acceleration.z *= P.z;
	...
	Filter_Butter2_LP_float_run( &disturbance_filter[0] , disturbance_Derivative.x );
	Filter_Butter2_LP_float_run( &disturbance_filter[1] , disturbance_Derivative.y );
	Filter_Butter2_LP_float_run( &disturbance_filter[2] , disturbance_Derivative.z );
	
	float outRoll = 	( ESO_AngularRate_get_EsMainPower( &ESO[0] ) + ESO[0].T * ( angular_acceleration_error.x * P.x + Ta1.x ) )/ESO[0].b;
	float outPitch =	( ESO_AngularRate_get_EsMainPower( &ESO[1] ) + ESO[1].T * ( angular_acceleration_error.y * P.y + Ta1.y ) )/ESO[1].b;
	float outYaw =		( ESO_AngularRate_get_EsMainPower( &ESO[2] ) + ESO[2].T * ( angular_acceleration_error.z * P.z + Ta1.z ) )/ESO[2].b;
...
//		output_throttle = throttle / cos_angle;
	...
	/*yaw output limit*/
		...
		/*find min yaw_scale*/
			...
			for( int i = 0 ; i < 4 ; ++i )
			{
				float current_out_yaw = ( (i==1) || (i==3) ) ? outYaw : -outYaw;
...
void init_ctrl_Attitude()
{
	...
	//Pitch初始设置
	Cfg_set_initial_PRYCtrl_b( 0 , 5.5f );
	Cfg_set_initial_PRYCtrl_T( 0 , 0.1f );
	Cfg_set_initial_PRYCtrl_P1( 0 , 15.0f );
	Cfg_set_initial_PRYCtrl_P2( 0 , 15.0f );
	Cfg_set_initial_PRYCtrl_TD4P( 0 , 25.0f );
	
	//Roll初始设置
	Cfg_set_initial_PRYCtrl_b( 1 , 5.5f );
	Cfg_set_initial_PRYCtrl_T( 1 , 0.1f );
	Cfg_set_initial_PRYCtrl_P1( 1 , 15.0f );
	Cfg_set_initial_PRYCtrl_P2( 1 , 15.0f );
	Cfg_set_initial_PRYCtrl_TD4P( 1 , 25.0f );
	
	//Yaw初始设置
	Cfg_set_initial_PRYCtrl_b( 2 , 1.0f );
	Cfg_set_initial_PRYCtrl_T( 2 , 0.005f );
	Cfg_set_initial_PRYCtrl_P1( 2 , 10.0f );
	Cfg_set_initial_PRYCtrl_P2( 2 , 10.0f );
	Cfg_set_initial_PRYCtrl_TD4P( 2 , 25.0f );
